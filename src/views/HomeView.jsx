  ...
  <div className="flex flex-col w-full min-h-[80vh] justify-between">
    ...
    <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12 w-full mb-12 lg:mb-20">
      <div className="flex-1 space-y-6 sm:space-y-8 z-10 w-full text-center lg:text-left">
        ...
        <div className="fade-in-section is-visible" style={{ transitionDelay: '200ms' }}>
        ...
        <div className="fade-in-section is-visible" style={{ transitionDelay: '400ms' }}>
          ...
          >
            ...
            <div className="absolute bottom-8 left-8 right-8 glass-panel rounded-2xl p-4 flex justify-between items-center backdrop-blur-md">
              <div>
                ...
                <p className="text-teal-400 font-extrabold">RS {db.combinations[0]?.price || '120'}</p>
              </div>
    ...
    <div className="w-full mb-12 lg:mb-20">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
        {db.shirts.slice(0, 4).map((product) => (
            <div key={product.id} className="glass-panel rounded-2xl sm:rounded-3xl overflow-hidden group hover:border-teal-400/40 transition-colors duration-300 flex flex-col h-full bg-[var(--glass-bg)] border border-[var(--glass-border)]">
              ...
              <div className="p-3 sm:p-5 flex flex-col flex-1 justify-between gap-3">
                <div onClick={() => onProductClick(product)} className="cursor-pointer">
                  ...
                  <p className="text-teal-400 font-bold text-sm sm:text-lg mt-1">RS {product.price}</p>
                </div>
                <div className="flex gap-2 w-full mt-auto">
                   <button onClick={(e) => { e.stopPropagation(); onAdd(product); }} className="flex-1 glass-button py-2 px-1 rounded-xl font-bold text-[10px] sm:text-sm whitespace-nowrap flex items-center justify-center gap-1 text-[var(--text-main)] hover:bg-teal-400/20 transition-colors">
                     <Plus size={14} /> Add
                   </button>
      ...
      <FadeIn>
        <div className="w-full glass-panel rounded-[2.5rem] p-8 sm:p-12 mb-12 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
          <div className="z-10 md:w-1/2 text-center md:text-left">
            <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-[var(--text-main)]">Join the Vintage Club</h2>
          ...
          <div className="z-10 md:w-1/2 w-full flex flex-col sm:flex-row gap-4 justify-end">
            <button onClick={() => navigate('signin')} className="glass-button bg-[var(--text-main)] text-[var(--bg-1)] px-8 py-4 rounded-full font-bold flex items-center justify-center gap-2 transition-transform hover:scale-105">
    ...
    <footer className="w-full glass-panel rounded-3xl p-6 sm:p-8 mt-auto flex flex-col md:flex-row justify-between items-center text-[var(--text-muted)] gap-6 border-t border-[var(--glass-border)] bg-[var(--glass-bg)]">
      <div className="text-center md:text-left">
        <h3 className="text-2xl font-bold text-[var(--text-main)] tracking-widest">VINTAGE</h3>
        <p className="text-sm mt-1">Premium Wears.</p>
      </div>
      ...
      <p className="text-sm text-center md:text-right">&copy; {new Date().getFullYear()} Vintage Clothing.<br className="hidden md:block" /> All rights reserved.</p>
    </footer>
  </div>
);
